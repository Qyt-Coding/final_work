...
public class KMeansRun {  
    ...
    public KMeansRun(int k, List<double[]> original_data) {
        this.kNum = k;
        this.original_data = original_data;//原始数据
        this.len = original_data.get(0).length; //向量的长度
        //检查规范
        check();//可以不用理
        //初始化点集。
        init();
...
//    }
        ...
        for (Cluster cluster : clusterSet) {
            ...
            for (int i = 0; i < len; i++) {
                for (int j = 0; j < point_list.size(); j++) {
                    sumAll[i] += point_list.get(j).getlocalArray()[i];//这个蔟中全部的x、y、z相加
                }
            }
            // 计算平均值
    ...
    public Set<Cluster> run() {
        Set<Cluster> clusterSet= chooseCenterCluster();//创建中间节点
